...
def read_as_df(basename, type="train", task="binary.classification", max_rows=None):
    ...
    try:
        X = pd.read_csv(
            basename + "_" + type + ".data",
            delim_whitespace=True,
            names=np.ravel(feat_name),
            index_col=False,
            nrows=max_rows,
        )
...
def data(filename, nbr_features=None, verbose=False):
    """ The 2nd parameter makes possible a using of the 3 functions of data reading (data, data_sparse,
    data_binary_sparse) without changing parameters.
    """
    if verbose:
